...
$query = "SELECT `f` FROM `voters` WHERE `id`='".$_SESSION['user_id']."'";
...
if(isset($_POST['financial'])) {
    ...
    if (!empty($financial)) {
        ...
        $query = "UPDATE `financial` SET `votes`=`votes`+1 WHERE `candidate_id`= '".mysql_real_escape_string($financial)."'";
        /* $query = "SELECT `votes` FROM `financial` WHERE `candidate_id`= '".mysql_real_escape_string($financial)."'";
         if ($query_run = mysql_query($query)) {

             while ($row = mysql_fetch_array($query_run)) {
                 $p_votes = $row['votes'];
                 $p_votes++;
                 $query ="UPDATE `financial` SET `votes`='$p_votes' WHERE `candidate_id`= '$financial'";*/
                if ($query_run = mysql_query($query)) {
                    $qry = "UPDATE `voters` SET `f` = '$financial' WHERE `id`='".$_SESSION['user_id']."'";
...
<div id="main-wrap">
    ...
    <div class="boxes-full">
        ...
        <div class="contacttitle">
            ...
            <h2 align="center">2016 National Association Of Nigerian Student Election:<br> financial General Ballot Paper</h2>
        ...
        <div class="boxes-padding fullpadding">
            ...
            <form action="<?php echo  $current_file;?> " method="post" name="ajaxcontactform" >
                ...
                $query = "SELECT `name`,`school`,`candidate_id`,`department`,`level`,`cgpa`,`image` FROM `financial`";
                ...
                while ($i<$count) {
                    ...
                    echo ' <fieldset>
                        <div class="col-md-3">';
                    echo "<input type='radio' name='financial' value=".$candidate_id.">";
                    echo '</div>
