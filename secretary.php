...
$query = "SELECT `sg` FROM `voters` WHERE `id`='".$_SESSION['user_id']."'";
...
if(isset($_POST['secretary'])) {
    ...
    //echo $vice_president;
    if (!empty($secretary)) {
      //  $query = "SELECT `votes` FROM `secretary` WHERE `candidate_id`= '".mysql_real_escape_string($secretary)."'";
        $query = "UPDATE `secretary` SET `votes`=`votes`+1 WHERE `candidate_id`= '".mysql_real_escape_string($secretary)."'";

        if ($query_run = mysql_query($query)) {

                    $qry = "UPDATE `voters` SET `sg` = '$secretary' WHERE `id`='".$_SESSION['user_id']."'";
    ...
    <div class="boxes-full">
        ...
        <div class="boxes-padding fullpadding">
            ...
            <form action="<?php echo  $current_file;?> " method="post" name="ajaxcontactform" >
                ...
                $query = "SELECT `name`,`school`,`candidate_id`,`department`,`level`,`cgpa`,`image` FROM `secretary`";
                ...
                while ($i<$count) {
                    ...
                    echo ' <fieldset>
                        <div class="col-md-3">';
                    echo "<input type='radio' name='secretary' value=".$candidate_id.">";
                    echo '</div>
